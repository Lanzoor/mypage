<script lang="ts">
    ...
    const depth = $derived(
        page.params.eggs ? page.params.eggs.split('/').filter(Boolean).length + 1 : 1
    );
    ...
    const messages = [
        ...
        'You found the Easter egg inside the Easter egg inside the Easter egg inside the Easter egg.<br>There are other things on this website, you know...',
        'You found the Easter egg inside the Easter egg inside the Easter egg inside the Easter egg inside the Easter egg. Why are you still doing this?',
